...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    async def handle_export_data(call):
        file_path = call.data.get("file_path", hass.config.path("ems_backup.json"))
        await hass.async_add_executor_job(manager.export_data, file_path)
        _LOGGER.info(f"EMS statistics exported to {file_path}")
    ...
    async def handle_import_data(call):
        ...
        if success:
            await manager.store.async_save(manager.data)
            manager._notify_update()
            _LOGGER.info(f"EMS statistics imported from {file_path}")
        else:
            _LOGGER.error(f"Failed to import EMS statistics from {file_path}")
...
async def async_remove_entry(hass: HomeAssistant, entry: ConfigEntry) -> None:
    ...
    store = Store(hass, STORAGE_VERSION, f"ems_{entry.entry_id}")
